...
def compare_PSNR(org, est, on_y=False, gray_scale=False):
    ...
    if on_y:
        return peak_signal_noise_ratio_y(np_to_pil(org), np_to_pil(est))
    if gray_scale:
...
def load_and_compare_psnr(fclean, fnoisy, crop_factor=1, on_y=False, eng=None):
    # matlab:
    if eng is not None:
        return eng.peak_signal_noise_ratio_y("../" + fclean, "../" + fnoisy, on_y, nargout=1)
    # load:
    ...
    return peak_signal_noise_ratio(img_np, img_noisy_np, on_y=on_y)
